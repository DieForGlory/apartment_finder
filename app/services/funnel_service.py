...
def get_target_funnel_metrics(start_date_str: str, end_date_str: str):
    ...
    # === Шаг 3: Разделение на Целевые/Нецелевые (без изменений) ===
    ...
    # Считаем, сколько ушло в следующий статус
    ...
    vstrecha_to_nesostoyalas_ids = {pid for pid in vstrecha_base_ids if
                                    lead_has_status(pid, 'Подбор', 'Визит не состоялся')}
    vstrecha_to_bron_ids = {pid for pid in vstrecha_base_ids if lead_has_status(pid, 'Бронь')}
    # Считаем, сколько ушло в следующий статус
    vstrecha_moved_on_ids = vstrecha_to_sostoyalas_ids | vstrecha_to_nesostoyalas_ids | vstrecha_to_bron_ids
    ...
    # Считаем, сколько ушло в следующий статус
    vizit_moved_on_ids = vizit_to_bron_ids
...
def get_dead_end_summary(start_date_str: str, end_date_str: str):
    ...
    final_statuses = {}
    for lead_id, status, custom_status in logs:
        # Просто перезаписываем статус для каждого ID; последний в отсортированном списке и будет финальным
    ...
    chart_labels = [item['name'] for item in summary[:10]]  # Берем топ-10 для наглядности
